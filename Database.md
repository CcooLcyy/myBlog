...
5. 数据是完整的
6. 增强安全性，可以保护数据库不被未经授权的用户访问。
# ANSI- SPARC 三层体系结构
虽然没有成为标准，但是它是理解DBMS的某些功能的基础。
外部层：用户从外部层观察数据
概念层：提供内外部层的映射以及必要的独立性。
内部层：DBMS和操作系统从内部层观察数据
使用此体系结构的目的是：
1. 访问的数据相同，但是用户可以根据需要自定数据视图。
2. 用户无需了解物理数据库的存储细节。
3. 因为物理与逻辑分离，所以DBA改变物理结构不会影响视图。
...
# MySQL使用
MySQL是关系型数据库管理系统（DBMS），可以使用多种编程语言。支持多线程。
## RDBMS术语（关系型数据库）
数据库：一些关联表的集合
数据表：表是数据的矩阵，二维的。
列：是相同的数据。
